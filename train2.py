...
from load_dataset import loaddatasplit2
from util import get_params_groups, create_lr_scheduler
from model import vgg, vivit, swin_transformer, timesformer_off, resnet
...
def get_argparser():
    ...
    parser.add_argument('--weights', default="", help='initial weights path')
    parser.add_argument('--time_patch', default=50, type=int)
...
if __name__ == '__main__':
    ...
    train_set = loaddatasplit2.Load_data(args, mode="train")
    validation_set = loaddatasplit2.Load_data(args, mode="valid")
    ...
    save_path = f"/root/projects/JJW_Model/contrast/weights/{model_name}_action9.pth"
    ...
    with open( log_file, 'a') as f:
        for epoch in range(args.epochs):
            ...
            for i, (raw_h, raw_v, dop_h, dop_v, action, action_confidence) in enumerate(trainloader):
                ...
                dop_h = dop_h.to(device)
                dop_v = dop_v.to(device)
                action = action.to(device).long()
                # action_confidence = action_confidence.to(device)
                ...
                for pred, true_label in zip(prediction, action):
                    if true_label in [0, 1, 2, 3, 4, 5, 6, 7, 8] and pred == true_label:
                        correct_sample += 1
            ...
            f.write(
                    f'[Epoch {epoch + 1}, tain] loss: {running_loss / all_sample:.3f}, train_accuracy: {correct_sample / all_sample:.5f}\n')
            f.write(
