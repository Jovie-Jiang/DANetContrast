...
class EfficientNet3D(nn.Module):
    ...
    def forward(self, raw_h,raw_v,drop_h,drop_v):
        x1 = torch.cat((raw_h, drop_h), dim=1)  # 合并水平通道输入
        x2 = torch.cat((raw_v, drop_v), dim=1)  # 合并垂直通道输入
        x1_features = self.extract_features(x1)
        x2_features = self.extract_features(x2)
        combined_features = x1_features + x2_features
        if self._global_params.include_top:
            # 全局池化
            combined_features = self._avg_pooling(combined_features)
            combined_features = combined_features.view(combined_features.size(0), -1)  # 展平
            combined_features = self._dropout(combined_features)  # Dropout
            combined_features = self._fc(combined_features)  # 全连接层
...
if __name__ == '__main__':
    x1 = torch.ones([1, 1, 70, 224, 224])
    x2 = torch.ones([1, 1, 70, 224, 224])
    x3 = torch.ones([1, 1, 70, 224, 224])
    x4 = torch.ones([1, 1, 70, 224, 224])

    model = EfficientNet3D.from_name("efficientnet-b0", override_params={'num_classes': 9}, in_channels=2)
    f = model(x1,x2,x3,x4)
    print(f)
